...
// Add this styled component for a scrollable column
...
function AllCarsPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    models: [],
    years: [],
    priceRange: [0, 1000000000] // Set a very high upper bound initially
  });
  ...
  const uniqueBrands = [...new Set(allCars.map(car => car.NameMMT.split(' ')[0]))].sort();
  const uniqueModels = [...new Set(allCars.map(car => car.Model))].sort();
  const uniqueYears = [...new Set(allCars.map(car => car.Yr))].sort((a, b) => b - a);
  const priceRange = [
    Math.min(...allCars.map(car => parseInt(car.Prc.replace(/,/g, '')))),
    Math.max(...allCars.map(car => parseInt(car.Prc.replace(/,/g, ''))))
  ];
